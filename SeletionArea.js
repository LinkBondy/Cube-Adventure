...
export class ArrayChartController {
    ...
    findCurrentArrayChart() {
        if (gameStates.currentSettingState === settingStates.ThemeColourSelection && gameStates.currentGameMode === gameMode.Settings)
            return 0
...
class ArrayChart {
    ...
    update() {
        var numberDrew = 0
        for (var col = 0; col < this.loopHeight; col++) {
            for (var row = 0; row < this.loopWidth; row++) {
                ...
                if (numberDrew === this.currentSelection)
                this.items[numberDrew].currentSelected = true
                else
                this.items[numberDrew].currentSelected = false
                numberDrew++
            }
        }
    }
...
class ArrayChartBox {
    ...
    Draw(row, col) {   
        if (!this.locked) {
            if (this.currentArrayChart.currentX === row && this.currentArrayChart.currentY === col)
                ...
                canvas.context.fillStyle = this.boarder 
    ...
    Update() {   
        if(this.checkedLocked !== undefined && this.checkedLocked !== null)
            this.checkedLocked()
    }
};
...
class ArrayChartImage {
    ...
    Draw(row, col) {
        ...
        if (!this.locked) 
            canvas.context.drawImage(this.image, this.imageStartX, this.imageStartY, this.imageStartWidth, this.imageStartHeight, this.x + 5, this.y + 5, this.currentArrayChart.sectionWidth - 10, this.currentArrayChart.sectionHeight - 10)
        else
            canvas.context.drawImage(images.LockedIcon, 0, 0, images.LockedIcon.width, images.LockedIcon.height, this.x + 5, this.y + 5, this.currentArrayChart.sectionWidth - 10, this.currentArrayChart.sectionHeight - 10)
    }
